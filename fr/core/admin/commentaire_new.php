...
if(!empty($_GET['c'])) { # Mode "answer"
	...
	if(!$plxAdmin->getCommentaires('/^'.$_GET['c'].'.xml$/',0,1)) { # Commentaire inexistant
		# On redirige
...
<?php if(!empty($_GET['a'])) : ?>
	<p><a href="commentaires_online.php?a=<?php echo $_GET['a']; ?>">&laquo; <?php echo L_BACK_TO_ARTICLE_COMMENTS ?></a></p>
<?php else : ?>
	<p><a href="commentaires_online.php">&laquo; <?php echo L_BACK_TO_COMMENTS ?></a></p>
<?php endif; ?>

<h2><?php echo L_CREATE_NEW_COMMENT ?> &laquo;<?php echo plxUtils::strCheck(plxUtils::strCut($aArt['title'],80)); ?>&raquo;)</h2>
...
<form action="commentaire_new.php?<?php echo plxUtils::strCheck($get) ?>" method="post" id="form_comment">
	<fieldset>
...
<?php if(isset($plxAdmin->plxRecord_coms)) : # On a des commentaires ?>
	...
	<?php while($plxAdmin->plxRecord_coms->loop()) : # On boucle ?>
		...
		<div class="comment type-<?php echo $plxAdmin->plxRecord_coms->f('type') ?><?php echo (isset($_GET['c']) AND $_GET['c']==$comId?' current':'') ?>" id="c<?php echo $plxAdmin->plxRecord_coms->f('numero'); ?>">
			<div class="info_comment">
				...
				le <?php echo plxDate::dateIsoToHum($plxAdmin->plxRecord_coms->f('date'), '#day #num_day #month #num_year(4) &agrave; #hour:#minute'); ?>
				 - <a href="commentaire.php<?php echo (!empty($_GET['a']))?'?c='.$comId.'&amp;a='.$_GET['a']:'?c='.$comId; ?>" title="<?php echo L_COMMENT_EDIT_TITLE ?>"><?php echo L_COMMENT_EDIT ?></a>
