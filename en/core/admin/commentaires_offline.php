...
# Hook Plugins
eval($plxAdmin->plxPlugins->callHook('AdminCommentsOfflinePrepend'));
...
if(!empty($_GET['a'])) {
	...
	$portee = L_COMMENTS_ARTICLE_SCOPE.' &laquo;'.plxUtils::strCheck(plxUtils::strCut($aArt['title'],80)).'&raquo;';
	$artRegex = $_GET['a'];
...
<h2><?php echo L_COMMENTS_OFFLINE_LIST . ' (' . $portee; ?>)</h2>

<?php eval($plxAdmin->plxPlugins->callHook('AdminCommentsOfflineTop')) # Hook Plugins ?>

<form action="commentaires_offline.php<?php echo !empty($_GET['a'])?'?a='.plxUtils::strCheck($_GET['a']):'' ?>" method="post" id="form_comments">
<table class="table">
<thead>
	<tr>
		...
		<th style="width:100px"><?php echo L_COMMENTS_LIST_ACTION ?></th>
	</tr>
</thead>
<tbody>
...
$coms = $plxAdmin->getCommentaires('/^_'.$artRegex.'.(.*).xml$/','rsort',$start,$plxAdmin->aConf['bypage_admin_coms'],'all');
if($coms) {
	$num=0;
	while($plxAdmin->plxRecord_coms->loop()) { # On boucle
		...
		echo '<td>&nbsp;<a href="commentaire.php?c='.$id.(!empty($_GET['a'])?'&amp;a='.$_GET['a']:'').'" title="'.L_COMMENT_EDIT_TITLE.'">'.$content.'</a></td>';
	...
	<tr>
		<td colspan="5">
			<?php plxUtils::printSelect('selection', array(''=> L_FOR_SELECTION, 'delete' => L_COMMENT_DELETE, 'online' => L_COMMENT_VALID), ''); ?>
...
<?php 
# Hook Plugins
eval($plxAdmin->plxPlugins->callHook('AdminCommentsOfflinePagination'));
...
if($coms) { # Si on a des commentaires (hors page)
	...
	if($plxAdmin->page > 1) # Si la page active > 1 on affiche un lien page precedente
		echo '<span><a href="'.$p_url.'" title="'.L_PAGINATION_PREVIOUS_TITLE.'"></a></span>';
...
<?php
# Hook Plugins
eval($plxAdmin->plxPlugins->callHook('AdminCommentsOfflineFoot'));
# On inclut le footer
