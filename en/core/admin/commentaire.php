...
if(!empty($_POST) AND !empty($_POST['comId'])) {
	# On définit vers quelle page il faut se rediriger
...
if(!$plxAdmin->getCommentaires('/^'.$_GET['c'].'.xml$/',0,1)) {
...
<?php if(!empty($_GET['a'])) : ?>
	<p><a href="./<?php echo $redirect.'?a='.$_GET['a']; ?>">&laquo; <?php echo L_BACK_TO_ARTICLE_COMMENTS ?></a></p>
<?php else : ?>
	<p><a href="./<?php echo $redirect; ?>">&laquo; <?php echo L_BACK_TO_COMMENTS ?></a></p>
<?php endif; ?>
...
<form action="commentaire.php<?php echo (!empty($_GET['a'])?'?a='.plxUtils::strCheck($_GET['a']):'') ?>" method="post" id="form_comment">
	<fieldset>
		<?php plxUtils::printInput('comId',$_GET['c'],'hidden'); ?>
		<p class="field"><label for="id_author"><?php echo L_COMMENT_AUTHOR_FIELD ?> :</label></p>
		<?php plxUtils::printInput('author',plxUtils::strCheck($plxAdmin->plxRecord_coms->f('author')),'text','40-255') ?>
		<p class="field"><label for="id_site"><?php echo L_COMMENT_SITE_FIELD ?></label></p>
		<?php plxUtils::printInput('site',plxUtils::strCheck($plxAdmin->plxRecord_coms->f('site')),'text','40-255') ?>
		<?php if($plxAdmin->plxRecord_coms->f('site') != '') : ?>
		<?php echo '<a href="'.$plxAdmin->plxRecord_coms->f('site').'">'.$plxAdmin->plxRecord_coms->f('site').'</a>' ?>
		<?php endif; ?>
		<p class="field"><label for="id_mail"><?php echo L_COMMENT_EMAIL_FIELD ?> :</label></p>
		...
		<?php endif; ?>
		<p class="field"><label for="id_content"><?php echo L_COMMENT_ARTICLE_FIELD ?> :</label></p>
		...
		<p class="center">
			<input class="bgred" type="submit" name="delete" value="<?php echo L_DELETE ?>" onclick="Check=confirm('<?php echo L_COMMENT_DELETE_CONFIRM ?>');if(Check==false) return false;"/>
			&nbsp;&nbsp;&nbsp;&nbsp;
			<?php if(preg_match('/^_/',$_GET['c'])) : ?>
				<input type="submit" name="mod" value="<?php echo L_COMMENT_PUBLISH_BUTTON ?>" /> 
			<?php else : ?>
				<input type="submit" name="mod" value="<?php echo L_COMMENT_OFFLINE_BUTTON ?>" />
